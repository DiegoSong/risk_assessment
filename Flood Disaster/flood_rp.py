...
def data_export(elements, site_id, start, end):
    ...
    df_select.iloc[:, 1] = df_select.iloc[:, 1]/10
...
class InfoInput:
    ...
    def __init__(self, input_spr, input_year, input_city, input_time, input_ele):
        self.input_spr = input_spr
...
while True:
    ...
    # 1.输入简单赔付率
    s_mat = InfoInput.input_spr()
    # 2.输入最低重现年数
    year = InfoInput.input_year()
    # 3.输入城市
    city = InfoInput.input_city()
    site = sl[sl['city_cn'] == city]['history_site_id'].values[0]
    # 4.输入起始终止时间
    start, end = InfoInput.input_time()
    # 5.输入要素
    elements = InfoInput.input_ele()
...
'''
start = time.clock()
df = data_export('SiteId,DateTime,prec20_20', '54511','2017-5-15','2017-6-15')
end = time.clock()
end - start
'''